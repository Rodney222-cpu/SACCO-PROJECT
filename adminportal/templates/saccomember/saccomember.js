...
$("#sacco_member_html_table").datagrid({
    ...
    columns: [[
        ...
        {field:'account_number', title:'{{ _('Account Number') }}',width:'11%'},
        {field:'fname', title:'{{ _('First Name') }}',width:'20%'},
        {field:'lname', title:'{{ _('Last Name') }}',width:'20%'},
        {field:'gender', title:'{{ _('Gender') }}',width:'20%'},
        ...
        {field:'options', title:'{{ _('Options') }}', width:'15%',
            formatter: function(value,row,index) {
                console.log(row);
                return '<a href="#" class="label-anchor" onclick="global_functions.saccomemberLaunchUpdateDialog(\''+index+'\');">Edit</a> ';
            }
        }
    ]]
});

global_functions.saccomemberLaunchUpdateDialog = function(index) {
    var dataRows = $("#sacco_member_html_table").datagrid("getData");
    global_functions.saccomemberAddNewSaccoMember(dataRows.rows[index]);
}


$("#sacco_member_html_table_toolbar_new_button").linkbutton({
    onClick: function () {
        global_functions.saccomemberAddNewSaccoMember({});
    }
});
...
$("#sacco_member_html_table_toolbar_delete_button").linkbutton({
    onClick: function () {
        global_functions.saccomembersDeleteSaccoMember({});
    }
});

global_functions.goBackToSaccos = function() {
    applayoutLoadRouteContent('{{ url_for('sacco.index')}}', 'applayout_route_id_saccos');
}

global_functions.saccomembersDeleteSaccoMember = function() {
        
    var rows = $("#sacco_member_html_table").datagrid('getSelections');
    if (rows.length < 1) {
    ...
    $.messager.confirm(
        ...
        function(r){
            if (r){
                $.ajax({
                    url: '{{ url_for("saccomember.deleteSaccoMembers") }}',
                    type: 'POST',
                    ...
                    success: function(data) {
                        if (data.status === "OK") {
                            ...
                            $("#sacco_member_html_table").datagrid('reload'); // Refresh the data grid
                        } else {
